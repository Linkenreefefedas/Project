...
# 訓練與評估流程實作包含 train_one_epoch, evaluate, cache_latents 等函數，負責訓練、驗證、緩存 latent 的具體步驟 
# ok train_one_epoch（訓練一輪）、evaluate（生成+評估）、cache_latents（緩存 latent）
...
def train_one_epoch(model, vae,
    ...
    for data_iter_step, (samples, labels) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            # new start
            # 三種消融：
            # - 只開 1+2：use_learned_schedule=True, learn_tau='off', learn_cfg='off'
            # - 只開 3+4：use_learned_schedule=False, learn_tau!='off' or learn_cfg!='off'
            # - 全開：三者都開
            if args.use_learned_schedule or args.learn_tau != 'off' or args.learn_cfg != 'off':
                loss = model.forward_joint(x, labels, total_steps_hint=args.num_iter)
            else:
                # 原本單步訓練
                loss = model(x, labels)
            # new end
...
def evaluate(model_without_ddp, vae, ema_params, args, epoch, batch_size=16, log_writer=None, cfg=1.0,
    ...
    for i in range(num_steps):
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                # new start
                if args.whether_learned:
                    sampled_tokens = model_without_ddp.sample_tokens_joint(
                            bsz=batch_size,
                            num_iter=args.num_iter,
                            labels=labels_gen,
                            default_tau=args.temperature,
                            default_cfg=args.cfg,
                            cfg_schedule=args.cfg_schedule,     # 'linear' or 'constant'
                            progress=False,
                            mode=args.infer_mode               # 'baseline' | 'policy' | 'guidance' | 'both'
                        )
                else:
