...
#VAE (Variational Autoencoder) 編碼器/解碼器，用於將圖片轉換為 latent 表示，或將 latent 還原為圖片 
# 它是 LDM（Latent Diffusion Model） 裡用的那種 連續 latent tokenizer，對應到 之前問的 KL-16 
'''
image ──► Encoder ──► quant_conv ──► DiagonalGaussianDistribution ──► latent z
latent z ──► post_quant_conv ──► Decoder ──► reconstructed image

'''
#輸入 256×256 → latent 16×16
#ok 就是VAE
